...
class ClusteringMachine(object):
    ...
    def decompose(self):
        ...
        if self.args.clustering_method == "metis":
            ...
            self.metis_clustering()     # mind this, didnt browse
        else:
    ...
    def metis_clustering(self):     # didnt browse
        """
    ...
    def transfer_edges_and_nodes(self):
        ...
        """
        for cluster in self.clusters:
            self.sg_nodes[cluster] = torch.LongTensor(self.sg_nodes[cluster])       # nodes_num * 1
            self.sg_edges[cluster] = torch.LongTensor(self.sg_edges[cluster]).t()   # t(): transpose --> 2*edges_num
            ...
            self.sg_features[cluster] = torch.FloatTensor(self.sg_features[cluster])# nodes_num * fdim
            self.sg_targets[cluster] = torch.LongTensor(self.sg_targets[cluster])   # nodes_num * 1
