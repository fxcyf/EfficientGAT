...
class ClusterGCNTrainer(object):
    ...
    def do_forward_pass(self, cluster):
        ...
        predictions = self.model(edges, features)           # self.model.forward(edges,features) ?; predictions in [N,out_channels]
        average_loss = torch.nn.functional.nll_loss(predictions[train_nodes], target[train_nodes])  # The negative log likelihood loss: nll_loss(input--(N,C),targer--(N))
