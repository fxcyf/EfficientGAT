...
def main():
    ...
    parser.add_argument("--results-path", nargs="?", default="../results/arxiv_results/",
                        help="path to store results. ")
    parser.add_argument("--clustering-path", nargs="?", default="../input/ogbn_arxiv/clustering/",
                        help="Clustering result txt")
    ...
    parser.add_argument("--cluster-number", type=int, default=200, help="Number of clusters extracted. Default is 10.")
    parser.add_argument("--cluster-batch", type=int, default=10,
    ...
    # nx.from_edgelist(pd.read_csv(args.edge_path).values.tolist())  # graph = nx.Graph(...edges)
    print(len(graph))
