import torch
import argparse
from clustering import ClusteringMachine
from clustergat import ClusterGATTrainer
from utils import tab_printer, graph_reader, feature_reader, target_reader, type_reader
import os

def main():
    """
    Parsing command line parameters, reading data, graph decomposition, fitting a ClusterGCN and scoring the model.
    """
    # parameters setting
    parser = argparse.ArgumentParser(description="Run .")
    parser.add_argument("--edge-path", nargs="?", default="../input/reddit_processed/edges.csv", help="Edge list csv.")
    parser.add_argument("--features-path", nargs="?", default="../input/reddit_processed/features.csv", help="Features json.")
    parser.add_argument("--target-path", nargs="?", default="../input/reddit_processed/target.csv", help="Target classes csv.")
    parser.add_argument("--type-map-path", nargs="?", default="../input/reddit_processed/",
                        help="path to test.csv, train.csv,valid.csv.")
    parser.add_argument("--clustering-path", nargs="?", default="../input/reddit_processed/", help="Clustering result txt")
    parser.add_argument("--clustering-method", nargs="?", default="metis",
                        help="Clustering method for graph decomposition. Default is the metis procedure.")
    parser.add_argument("--results-path", nargs="?", default="../results/reddit_results/",
                        help="path to store results. ")
    parser.add_argument("--seed", type=int, default=42, help="Random seed for train-test split. Default is 42.")
    parser.add_argument('--no_cuda', action='store_true', default=False, help='Use CUDA training.')  # 无需带参数
    parser.add_argument("--epochs", type=int, default=1500, help="Number of training epochs. Default is 200.")
    parser.add_argument("--dropout", type=float, default=0.5, help="Dropout parameter. Default is 0.5.")
    parser.add_argument("--learning-rate", type=float, default=0.001, help="Learning rate. Default is 0.01.")
    parser.add_argument("--test-ratio", type=float, default=0.1, help="Test data ratio. Default is 0.1.")
    parser.add_argument("--cluster-number", type=int, default=1500, help="Number of clusters extracted. Default is 10.")
    parser.add_argument("--cluster-batch", type=int, default=20,
                        help="number of clusters to form a batch. Default is one")

    parser.add_argument('--hidden', type=int, default=8, help='Number of hidden units.')
    parser.add_argument('--nb_heads', type=int, default=8, help='Number of head attentions.')
    parser.add_argument('--patience', type=int, default=100, help='Patience')

    args = parser.parse_args()  # args = parser.parse_args()

    torch.manual_seed(args.seed)  # random seed
    tab_printer(args)
    # load data and metis
    # if not os.path.exists(args.clustering_path):
    graph = graph_reader(args.edge_path)  # graph = nx.Graph(...edges)
    features = feature_reader(args.features_path)  # features = coo_matrix(...)
    target = target_reader(args.target_path)  # target = np.array(size=(-1,1))
    type_map = type_reader(args.type_map_path)

    clustering_machine = ClusteringMachine(args, graph, features, target, type_map)
    clustering_machine.decompose()

    myGAT = ClusterGATTrainer(args, clustering_machine)
    myGAT.train()
    myGAT.test()


if __name__ == "__main__":
    main()
